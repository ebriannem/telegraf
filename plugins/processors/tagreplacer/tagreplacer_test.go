...
func TestBasic(t *testing.T) {
	r := TagReplacer {
		TagName: "name1",
		TrimValues: [][]string{[]string{"a"}, []string{"b"}},
	}
...
func TestBasic2(t *testing.T) {
	r := TagReplacer {
		TagName: "name1",
		TrimValues: [][]string{[]string{"abcbc"}},
	}
...
func TestReplaces(t *testing.T) {
	r := TagReplacer {
		TagName: "name1",
		TrimValues: [][]string{[]string{"a", "b"}, []string{"c", "d", "e"}},
	}
	m1 := newMetric("foo1", map[string]string{"name1": "abbbb"}, nil)
	m2 := newMetric("foo2", map[string]string{"name1": "123ddd"}, nil)
	m3 := newMetric("foo3", map[string]string{"name1": "efg"}, nil)

	results := r.Apply(m1, m2, m3)

	m12 := newMetric("foo1", map[string]string{"name1": "a"}, nil)
	m22 := newMetric("foo2", map[string]string{"name1": "c"}, nil)
	m32 := newMetric("foo3", map[string]string{"name1": "c"}, nil)
